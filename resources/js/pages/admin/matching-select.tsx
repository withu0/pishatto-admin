...
const mockMatching = [
    { id: 1, guest: '山田 太郎', call: 'コールA', cast: '高橋 美咲', status: '選定済', date: '2024-07-01' },
    { id: 2, guest: '佐藤 花子', call: 'コールB', cast: '田中 直樹', status: '未選定', date: '2024-06-15' },
];

export default function AdminMatchingSelect() {
    const [search, setSearch] = useState('');
    const filtered = mockMatching.filter(
        (m) => m.guest.includes(search) || m.cast.includes(search) || m.call.includes(search)
    );
    return (
        <AppLayout breadcrumbs={[{ title: 'マッチング選定', href: '/admin/matching-select' }]}>
            <Head title="マッチング選定" />
            <div className="p-6">
                <h1 className="text-2xl font-bold mb-4">マッチング選定</h1>
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between gap-4 pb-2">
                        <CardTitle>マッチング選定一覧</CardTitle>
                        <Button size="sm" className="gap-1"><Plus className="w-4 h-4" />新規登録</Button>
                    </CardHeader>
                    <CardContent>
                        <div className="mb-4 flex items-center gap-2">
                            <Input
                                placeholder="ゲスト・キャスト・コールで検索"
                                value={search}
                            ...
                            <table className="min-w-full text-sm border">
                                <thead>
                                    <tr className="bg-muted">
                                        ...
                                        <th className="px-3 py-2 text-left font-semibold">コール</th>
